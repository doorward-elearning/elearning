...
class Watermarks extends Component<Props, State> {
  ...
  constructor(props: Props) {
    ...
    if (interfaceConfig.filmStripOnly) {
      ...
      showJitsiWatermarkForGuests = interfaceConfig.SHOW_WATERMARK_FOR_GUESTS;
    }

    this.state = {
      brandWatermarkLink: showBrandWatermark ? interfaceConfig.BRAND_WATERMARK_LINK : '',
      jitsiWatermarkLink: showJitsiWatermark || showJitsiWatermarkForGuests ? interfaceConfig.JITSI_WATERMARK_LINK : '',
      showBrandWatermark,
      showJitsiWatermark,
  ...
  _canDisplayJitsiWatermark() {
    ...
    const { _isGuest, _readyToDisplayJitsiWatermark, _welcomePageIsVisible } = this.props;

    return (
      (_readyToDisplayJitsiWatermark && (showJitsiWatermark || (_isGuest && showJitsiWatermarkForGuests))) ||
  ...
  _getBackgroundImageStyle() {
    const { _customLogoUrl, _isVpaas, defaultJitsiLogoURL } = this.props;
    let style = 'none';
    ...
    if (_isVpaas) {
      ...
      style = `url(${_customLogoUrl || defaultJitsiLogoURL || interfaceConfig.DEFAULT_LOGO_URL})`;
    }
