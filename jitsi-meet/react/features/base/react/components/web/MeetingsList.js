    ...
    if (meetings) {
      return (
        <Container className="meetings-list">
          {meetings.length === 0 ? listEmptyComponent : meetings.map(this._renderItem)}
        </Container>
      );
    }
    ...
    return (
      <Container className={rootClassName} key={index} onClick={onPress}>
        <Container className="left-column">
          ...
          <Text>{_toTimeString(time)}</Text>
        </Container>
        <Container className="right-column">
          <Text className="title">{title}</Text>
          {hideURL || !url ? null : <Text>{url}</Text>}
          {typeof duration === 'number' ? <Text>{getLocalizedDurationFormatter(duration)}</Text> : null}
        </Container>
        <Container className="actions">{elementAfter || null}</Container>
      </Container>
    );
  }
}
