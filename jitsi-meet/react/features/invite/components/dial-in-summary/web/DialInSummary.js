...
class DialInSummary extends Component<Props, State> {
  ...
  render() {
    ...
    if (loading) {
      ...
      contents = [
        conferenceID ? (
          <ConferenceID conferenceID={conferenceID} conferenceName={this.props.room} key="conferenceID" />
        ) : null,
        <NumbersList
  ...
  _getConferenceID() {
    ...
    return doGetJSON(`${dialInConfCodeUrl}?conference=${room}@${mucURL}`, true).catch(() =>
      Promise.reject(this.props.t('info.genericError'))
    );
  }
  ...
  _getNumbers() {
    ...
    return doGetJSON(`${dialInNumbersUrl}${URLSuffix}`, true).catch(() =>
      Promise.reject(this.props.t('info.genericError'))
    );
  }
