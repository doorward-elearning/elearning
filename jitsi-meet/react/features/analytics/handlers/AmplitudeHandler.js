...
export default class AmplitudeHandler extends AbstractHandler {
  ...
  setUserProperties(userProps) {
    if (this._enabled) {
      amplitude.getInstance(this._amplitudeOptions).setUserProperties(userProps);
    }
  }
  ...
  sendEvent(event) {
    ...
    amplitude.getInstance(this._amplitudeOptions).logEvent(this._extractName(event), event);
  }
