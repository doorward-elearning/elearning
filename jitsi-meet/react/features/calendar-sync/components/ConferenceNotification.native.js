...
class ConferenceNotification extends Component<Props, State> {
  ...
  render() {
    ...
    if (event) {
      const now = Date.now();
      const label =
        event.startDate < now && event.endDate > now ? 'calendarSync.ongoingMeeting' : 'calendarSync.nextMeeting';
    ...
    if (_eventList && _eventList.length) {
      ...
      for (const event of _eventList) {
        ...
        if (eventUrl && eventUrl !== _currentConferenceURL) {
          if (
            (!eventToShow && event.startDate > now && event.startDate < now + ALERT_MILLISECONDS) ||
