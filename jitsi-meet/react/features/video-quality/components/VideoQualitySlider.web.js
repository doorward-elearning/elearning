...
class VideoQualitySlider extends Component<Props> {
  ...
  render() {
    ...
    return (
      <div className={classNames}>
        ...
        <div className={warning ? '' : 'hide-warning'}>{warning}</div>
  ...
  _renderAudioOnlyLockedMessage() {
    ...
    return (
      <InlineMessage title={t('videoStatus.onlyAudioAvailable')}>{t('videoStatus.onlyAudioSupported')}</InlineMessage>
    );
  }
  ...
  _mapCurrentQualityToSliderValue() {
    ...
    const matchingOption = _sliderOptions.find(({ videoQuality }) => videoQuality === _sendrecvVideoQuality);
