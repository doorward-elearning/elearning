...
const googleApi = {
    ...
    return this.get().then(
      (api) =>
        new Promise((resolve, reject) => {
          const scope = `${enableYoutube ? GOOGLE_SCOPE_YOUTUBE : ''} ${
            enableCalendar ? GOOGLE_SCOPE_CALENDAR : ''
          }`.trim();
  ...
  signOut() {
    return this.get().then(
      (api) =>
        api.auth2 && api.auth2.getAuthInstance && api.auth2.getAuthInstance() && api.auth2.getAuthInstance().signOut()
    );
  },
