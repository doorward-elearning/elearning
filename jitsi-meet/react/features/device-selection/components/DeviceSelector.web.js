...
class DeviceSelector extends Component<Props> {
  ...
  render() {
    ...
    const defaultSelected = items.find((item) => item.value === this.props.selectedDeviceId);
  ...
  _createDropdownTrigger(triggerText) {
    return (
      <div className="device-selector-trigger">
        ...
        <ChevronDownIcon label="expand" size="large" />
      </div>
    );
  }
  ...
  _createDropdown(options) {
    const triggerText = (options.defaultSelected && options.defaultSelected.content) || options.placeholder;
