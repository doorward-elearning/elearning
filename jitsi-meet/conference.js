...
import { getBackendSafePath, getJitsiMeetGlobalNS } from './react/features/base/util';
...
export default {
  ...
  createInitialLocalTracks(options = {}) {
    ...
    if (!options.startWithVideoMuted && !options.startAudioOnly && !options.startScreenSharing) {
      initialDevices.push('video');
      requestedVideo = true;
    }

    JitsiMeetJS.mediaDevices.addEventListener(JitsiMediaDevicesEvents.PERMISSION_PROMPT_IS_SHOWN, (browser) =>
      APP.store.dispatch(mediaPermissionPromptVisibilityChanged(true, browser))
    );
  ...
  async init({ roomName }) {
    const initialOptions = {
      ...
      startWithAudioMuted:
        config.startWithAudioMuted || config.startSilent || isUserInteractionRequiredForUnmute(APP.store.getState()),
      startWithVideoMuted: config.startWithVideoMuted || isUserInteractionRequiredForUnmute(APP.store.getState()),
    };
  ...
  _setupListeners() {
    ...
    room.on(JitsiConferenceEvents.PARTCIPANT_FEATURES_CHANGED, (user) => APP.UI.onUserFeaturesChanged(user));
    ...
    room.on(JitsiConferenceEvents.PARTICIPANT_PROPERTY_CHANGED, (participant, name, oldValue, newValue) => {
      switch (name) {
            ...
            Promise.all(
              Object.keys(requestedInput).map((mediaType) => {
                if (requestedInput[mediaType]) {
                  const useStream =
                    mediaType === 'audio' ? this.useAudioStream.bind(this) : this.useVideoStream.bind(this);
