 ...
 */
export class MigrationGenerateCommand<T, U> implements yargs.CommandModule {
  ...
  public async handler(args: yargs.Arguments<U>) {
    ...
    try {
      ...
      if (args.check) {
        console.log(
          chalk.yellow(`Unexpected changes in database schema were found in check mode:\n\n${chalk.white(fileContent)}`)
        );
        process.exit(1);
      }
