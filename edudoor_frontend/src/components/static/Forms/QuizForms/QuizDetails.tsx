...
const QuizDetails: React.FunctionComponent<QuizDetailsProps> = ({ form }) => {
  return (
      ...
      <div className="quiz-details">
        <div>
          <Panel noBackground>
            ...
            <IfElse condition={form.formikProps?.values.content.options.timeLimit.allow}>
          ...
          <Panel noBackground>
            ...
            <IfElse condition={form.formikProps?.values.content.options.questions.oneAtATime}>
              <Checkbox name="content.options.questions.lockAfterAnswering" label="Lock questions after answering" />
            </IfElse>
          </Panel>
          <Panel noBackground>
            <Header size={3}>Attempts</Header>
            <Checkbox name="content.options.attempts.multiple" label="Allow multiple attempts" />
            <IfElse condition={form.formikProps?.values.content.options.attempts.multiple}>
              <React.Fragment>
                <DropdownSelect
                  ...
                  icon="timelapse"
                />
                <NumberField name="content.options.attempts.allowed" label="Allowed attempts" min={1} />
              </React.Fragment>
            </IfElse>
          </Panel>
          <Panel noBackground>
            ...
            <IfElse condition={form.formikProps?.values.content.options.restrictions.accessCode.require}>
          ...
          <Panel noBackground>
            ...
            <IfElse condition={form.formikProps?.values.content.options.responses.show}>
              <React.Fragment>
                <Checkbox name="content.options.responses.frequency.onlyOnce" label="Only once after each attempt" />
                <Checkbox
                  name="content.options.responses.frequency.range.allow"
                  label="Let students see the correct answers"
                />
                <IfElse condition={form.formikProps?.values.content.options.responses.frequency.range.allow}>
                  <React.Fragment>
                    <DateInput
