...
class Link extends Component<LinkProps, any> {
  ...
  getCurrentValues = () => {
    ...
    if (currentEntity && (contentState.getEntity(currentEntity) as any).get('type') === 'LINK') {
      currentValues.link = {};
      const entityRange = currentEntity && getEntityRange(editorState, currentEntity);
      currentValues.link.target = currentEntity && (contentState.getEntity(currentEntity) as any).get('data').url;
      currentValues.link.targetOption =
        currentEntity && (contentState.getEntity(currentEntity) as any).get('data').targetOption;
