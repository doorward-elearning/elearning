...
export class CanvasWhiteboardComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  private _initCanvasServiceObservables(): void {
    ...
    this._canvasWhiteboardServiceSubscriptions.push(
      this._canvasWhiteboardService.canvasUndoSubject$.subscribe(updateUUD => this._undoCanvas(updateUUD))
    );
    this._canvasWhiteboardServiceSubscriptions.push(
      this._canvasWhiteboardService.canvasRedoSubject$.subscribe(updateUUD => this._redoCanvas(updateUUD))
    );

    this._registeredShapesSubscription = this._canvasWhiteboardShapeService.registeredShapes$.subscribe(shapes => {
      if (
        !this.selectedShapeConstructor ||
        !this._canvasWhiteboardShapeService.isRegisteredShape(this.selectedShapeConstructor)
      ) {
  ...
  private _calculateCanvasWidthAndHeight(): void {
    const { width, height } = this.canvas.nativeElement.parentNode.getBoundingClientRect();
    this.context.canvas.width = width;
    if (this.aspectRatio) {
      this.context.canvas.height = width * this.aspectRatio;
    } else {
  ...
  private _resetIncompleteShapeCanvas() {
    this._incompleteShapesCanvasContext.clearRect(
      0,
      0,
      this._incompleteShapesCanvasContext.canvas.width,
      this._incompleteShapesCanvasContext.canvas.height
    );
    this._incompleteShapesCanvasContext.fillStyle = 'transparent';
    this._incompleteShapesCanvasContext.fillRect(
      0,
      0,
      this._incompleteShapesCanvasContext.canvas.width,
      this._incompleteShapesCanvasContext.canvas.height
  ...
  private _addCurrentShapeDataToAnUpdate(update: CanvasWhiteboardUpdate) {
    ...
    if (!update.selectedShapeOptions) {
      ...
      update.selectedShapeOptions = Object.assign(
        new CanvasWhiteboardShapeOptions(),
  ...
  downloadCanvasImage(
    ...
    if (window.navigator.msSaveOrOpenBlob === undefined) {
      ...
      let fileName = customFileName
        ...
        ? this.downloadedFileName
        : 'canvas_drawing_' + new Date().valueOf();
  ...
  private _saveCanvasBlob(blob: Blob, returnedDataType: string = 'image/png'): void {
    window.navigator.msSaveOrOpenBlob(
      blob,
      'canvas_drawing_' + new Date().valueOf() + this._generateDataTypeString(returnedDataType)
    );
  }
