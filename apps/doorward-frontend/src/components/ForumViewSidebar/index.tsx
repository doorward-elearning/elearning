...
import { fetchForumManagersAction, fetchForumMemberListAction } from '../../reducers/forums/actions';
...
const ForumViewSidebar: React.FunctionComponent<ForumViewSidebarProps> = props => {
  const members = useSelector((state: State) => state.forums.memberList);
  const managers = useSelector((state: State) => state.forums.managersList);
  const match: any = useRouteMatch<{ forumId: string }>();
  const routes = useRoutes();
  const fetchMembers = useAction(fetchForumMemberListAction);
  const fetchManagers = useAction(fetchForumManagersAction);
  const forumId = match.params.forumId;

  useEffect(() => {
    fetchMembers(forumId);
    fetchManagers(forumId);
  }, []);

  const MAX_MEMBERS = 3;
  const MAX_MANAGERS = 3;
  return (
    <div className="forum-view-sidebar">
      <RoleContainer roles={[Roles.MODERATOR]}>
        ...
        >
          <WebComponent
            ...
            message="No members have been added to the forum yet."
            size="medium"
            actionMessage="Create a new member"
            onAction={(): void =>
              routes.navigate(routes.routes.addForumMember, {
                forumId,
              })
            }
          >
            {(members): JSX.Element => (
              <List>
                ...
                <ListItem>
                  <Link
                    to={routes.routes.forumMembers.withParams({
                      forumId: forumId,
                    })}
                  >
                    View all
        ...
        <div style={{ marginTop: 'var(--padding)' }}>
          <Accordion
            action={() => <Button mini bordered icon="add" onClick={props.addForumManagerModal.openModal} />}
            title={() => <Header size={5}>Forum Managers</Header>}
            open
          >
            <WebComponent
              data={managers.data.managers}
              loading={members.fetching}
              message="No managers have been added to the forum yet."
              size="medium"
...
export interface ForumViewSidebarProps {
  addMemberModal: UseModal;
  addForumManagerModal: UseModal;
}

export default ForumViewSidebar;
