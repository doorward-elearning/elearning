...
import { fetchMembersNotRegisteredAction, registerMembers } from '../../../reducers/courses/actions';
...
const ChooseMemberForm: React.FunctionComponent<ChooseMemberFormProps> = props => {
  const memberList = useSelector((state: State) => state.courses.notRegistered);
  ...
  useEffect(() => {
    fetchMembers(courseId, { search: props.search });
          ...
          <Panel plain>
            <Row style={{ alignItems: 'start' }}>
              <ChooseItemsForm
                ...
                onRemoveFilter={props.onClearSearch}
                hasSearch={!!props.search}
                createData={values => [courseId, { members: createMembersFromGroups(values) }]}
                columns={{
                  name: 'Group name',
                  members: 'Members',
                }}
