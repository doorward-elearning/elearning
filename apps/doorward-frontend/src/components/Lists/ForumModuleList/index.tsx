...
import {
  deleteForumModuleAction,
  reorderForumModules,
  updateForumModuleAction,
  ...
  return (
    <Panel>
      <Accordion
        title={(): JSX.Element => (
          <EditableLabelForm
            submitAction={updateForumModuleAction}
            state={updateModule}
...
const ForumModuleList: React.FunctionComponent<ForumModuleListProps> = ({ forum }) => {
  const updateModule = useSelector((state: State) => state.forums.updateModule);
  const action = useAction(reorderForumModules);
  const hasRole = useRoleManager([Roles.MODERATOR]);
  const [handleDrop] = useModuleDrop(forum.id, action);
  const state = useSelector((state: State) => state.forums.deleteModule);
  const deleteModuleModal = useModal();
  const [moduleToDelete, setModuleToDelete] = useState();

  return (
    <div className="forum-module-list">
      <WebConfirmModal
        args={[moduleToDelete]}
        title="Delete Module"
        useModal={deleteModuleModal}
        action={deleteForumModuleAction}
        state={state}
      >
        <p>Are you sure you want to delete this module?</p>
      </WebConfirmModal>
      <WebComponent data={forum.modules} loading={false}>
...
export interface ForumModuleListProps {
  forum: Forum;
}
...
export default ForumModuleList;
