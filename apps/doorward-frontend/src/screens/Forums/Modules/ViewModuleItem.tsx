...
import { fetchModuleItem } from '../../../reducers/forums/actions';
...
const ViewModuleItem: React.FunctionComponent<ViewModulePageProps> = props => {
  ...
  const state = useSelector((state: State) => state.forums.moduleItem);
  ...
  useEffect(() => {
    if (forum.data.forum) {
      const currentModule = forum.data.forum.modules.find(module => module.id === match.params.moduleId);
      if (currentModule) {
        setModule(currentModule);
        routes.setTitle(routes.viewModuleItem.id, currentModule.title);
      }
    }
  }, [forum.data.forum, match.params.itemId]);
              ...
              {module && (
                <React.Fragment>
                  ...
                  <IfElse condition={item.type === 'Quiz'}>
                    <EditableView
                      viewerView={<QuizView quiz={item} onCancel={() => routes.navigate(routes.viewForum, params)} />}
                      creatorView={
                        <CreateQuizForm
                          onSuccess={() => {}}
