...
import { fetchModuleItems } from '../../../reducers/conferences/actions';
...
const AssignmentsList: React.FunctionComponent<AssignmentsListProps> = (props): JSX.Element => {
  const state = useSelector((state: State) => state.conferences.moduleItemList);

  const routes = useRoutes();
  const [conferenceId] = useViewConference();
  usePageResource('conferenceId', fetchModuleItems, ['Assignment']);
  return (
