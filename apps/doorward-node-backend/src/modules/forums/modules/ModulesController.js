...
class ModulesController {
  static async getForumModules(req) {
  ...
  static async addForumModule({ body, params }) {
    let module = await ModulesController.createModule(params.forumId, body);
    module = await models.Module.findByPk(module.id, {
      include: ModuleInclude,
    });
    return [201, { module }, 'Module has been added to the forum'];
  }

  static async updateForumModule({ body, params }) {
  ...
  static async deleteForumModule({ params }) {
  ...
  static async getForumModule({ params }) {
  ...
  static async createModule(forumId, moduleData) {
  ...
  static async getForumModuleItems(req) {
    const { params } = req;
    const items = await models.ModuleItem.findAll({
      where: {
        ...req.searchFields,
